...
async function testMongoConnection() {
  ...
  const sanitizedUri = uri.replace(/\/\/([^:]+):([^@]+)@/, "//[username]:[password]@");
  ...
  if (!uri.startsWith('mongodb://') && !uri.startsWith('mongodb+srv://')) {
    ...
    try {
      ...
      console.error('❌ Connection still failed with the fixed URI:', fixError.message);
      console.error('Please check your MongoDB service and network connectivity.');
      process.exit(1);
    }
  ...
  try {
    ...
    if (error.name === 'MongoParseError') {
      ...
      console.error('Make sure it follows the format: mongodb+srv://username:password@host/database');
