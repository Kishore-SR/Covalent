...
async function setupVercelEnv() {
  ...
  if (mongoUri) {
    console.log(`Current MONGODB_URI: ${mongoUri.replace(/\/\/([^:]+):([^@]+)@/, "//[username]:[password]@")}`);
  ...
  if (jwtSecret) {
    ...
    console.log(`Generated random JWT secret: ${generatedSecret.substring(0, 10)}...`);
  ...
  try {
    ...
    const { stdout, stderr } = await execAsync(`vercel env import < ${envFile}`);
    console.log(stdout);
    ...
    console.error('Please try setting them manually with the vercel env add command');
  }
